...
func test_place_current_at_open_cell() -> void:
	...
	assert_true(gs.place(2, 2, 0), "placement on an open cell succeeds")
	assert_eq(gs.pipe_at(2, 2), expected, "placed piece is the forced current")
	assert_eq(gs.pipe_rot_at(2, 2), 0)
...
func test_place_rejected_on_blocked() -> void:
	...
	assert_false(gs.place(1, 1, 0), "cannot place on a blocked cell")
...
func test_place_rejected_on_bomb() -> void:
	...
	var gs = GameState.new(b, PieceQueue.new(1))
	assert_false(gs.place(1, 1, 0), "cannot place on a bomb cell")


func test_place_rejected_in_flow_phase() -> void:
	var gs = _gs()
	gs.go()
	assert_false(gs.place(2, 2, 0), "no placement during FLOW")


func test_dry_overwrite_allowed() -> void:
	var gs = _gs()
	assert_true(gs.place(2, 2, 0))
	assert_true(gs.place(2, 2, 1), "overwriting a DRY pipe is allowed")
...
func test_wet_overwrite_rejected() -> void:  # control
	...
	assert_true(gs.place(2, 2, 0))
	gs.mark_wet(2, 2)
	assert_false(gs.place(2, 2, 1), "cannot overwrite a WET pipe")
