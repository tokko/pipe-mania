...
- Gate: `tools/run-gate.ps1` (headless GUT). Green; the test count is intentionally not hard-coded here because it drifts with focused additions. One intentional quarantined control remains.
- **Full-game shell SHIPPED (`0b719e1` + teardown BLOCKER fix):** splash → start menu → game → run-over, plus leaderboard/settings modals — code-built `CanvasLayer` views (`scripts/view/*_view.gd` + shared `scripts/view/ui_style.gd`) driven by `scripts/screen_controller.gd` FSM (mounted only on the non-test branch; `PIPE_TEST` entry untouched). Local top-10 leaderboard (`SaveStore`, 3-initial entry). Monetization UX: callback-driven Revive plus between-run interstitials; Remove Ads is not offered because Play Billing is deferred. Procedural audio (`audio_cues.gd` bakes `AudioStreamWAV`). HUD shrunk to a responsive bottom GO/Menu bar. Whole flow verified on-device (`3A191FDJH000K4`) incl. Revive-resume + place→Menu (no crash).
- **Ads implemented and tested:** the current scope is rewarded revive and between-run interstitials. Poing Studios AdMob plugin v4.3.1 is installed and enabled; Android Test uses the Gradle export with Google demo ads. Runtime and the project exporter override select the App ID from `scripts/monetization_config.gd`; the legacy/default export receives no native ads metadata. Desktop/headless/`PIPE_TEST` remain deterministic STUB, and LIVE fails closed until production IDs are complete and non-demo. See `docs/MONETIZATION_SETUP.md`.
- **Release boundary:** production account setup, privacy publishing, release signing, and store publishing remain human/account-gated. Billing, Remove Ads, cosmetics, and online leaderboards are deferred and not offered in this scope.
...
## Next session
- **Production ads and publishing (needs your accounts):** follow `docs/MONETIZATION_SETUP.md`. Supply the
  production App ID, rewarded/interstitial IDs, privacy-policy and `app-ads.txt` URLs, release signing, and
  Play publishing inputs. Then create the production export path and verify the signed release on-device.
